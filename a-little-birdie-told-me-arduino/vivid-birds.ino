/**
 * Project: Neopixel Bird Project
 * Description: 
 * Reads MP3 files randomly from an SD card and uses a spectrum analyser
 * and neopixels to display lights corresponding to the noises
 * 
 * @author Kaan Seven
 * @version 0.6 24/4/2018
 */

// When in doubt look for BORIS
...
#define NEOPIXEL1_PIN    5   // Digital PWM pin for neopixel strip
#define NEOPIXEL2_PIN    9   // Digital PWM pin for neopixel strip
#define NEOPIXEL3_PIN    10   // Digital PWM pin for neopixel strip

#define SWITCH1_PIN     0   // Digital pin to read rotary switch
#define SWITCH2_PIN     1   // Digital pin to read rotary switch
...
#define COLOUR_PIN      2   // Analogue pin for potentiometer to adjust colour
#define INTERVAL_PIN    5   // Analogue pin for potentiometer to adjust time intervals

const int numOfPixels = 10  ; // Number of neopixels - keep each strip the same 
const int middleNum = numOfPixels/2; // 
Adafruit_NeoPixel strip1 = Adafruit_NeoPixel(numOfPixels, NEOPIXEL1_PIN);
Adafruit_NeoPixel strip2 = Adafruit_NeoPixel(numOfPixels, NEOPIXEL2_PIN); 
Adafruit_NeoPixel strip3 = Adafruit_NeoPixel(numOfPixels, NEOPIXEL3_PIN); 
Adafruit_VS1053_FilePlayer musicPlayer = Adafruit_VS1053_FilePlayer(SHIELD_RESET, SHIELD_CS, SHIELD_DCS, DREQ, CARDCS); 

const int switchMin = 1; 
int switchMax = 1; 
int numOfMP3s = 0;
const int tickInterval = 10; 

const unsigned long colourSelectionTime = 20000;  // Starting time to select colour (ms)
const unsigned long minIntervalTime = 10000;    // Minimum time for chaser to mvoe one position (ms)
const unsigned long maxIntervalTime = 600000;  // Maximum time for chaser to move one position (ms)
const long addTick = maxIntervalTime * tickInterval / minIntervalTime - tickInterval; 

float colourArray[] = { 255, 255, 200 };  
const float offArray[] = { 0, 0, 0 }; 

const unsigned long randSeed = 85030; //change for each arduino setup 

const int frequencyBand = 10; // pick between 1 at 14 for the selected frequency band 


void setup() {
  randomSeed(randSeed);
  pinMode(NEOPIXEL1_PIN, OUTPUT);
  pinMode(NEOPIXEL2_PIN, OUTPUT);
  pinMode(NEOPIXEL3_PIN, OUTPUT);
  ...
  else if (digitalRead(SWITCH2_PIN) == 0) {
    switchMax = 3;
  }
  else if (digitalRead(SWITCH3_PIN) == 0) {
    switchMax = 4;
  }*/
  numOfMP3s = numberOfMP3s(SD.open("/"));
  //colourSelection();
  delay(random(0,120000));
}

void loop() { 
  File folder = SD.open("/");
  int randomSongN = random(0, numOfMP3s)+1;
  folder.rewindDirectory();
  File random_file = selectFileN(folder, randomSongN);
  folder.close();
  if (! musicPlayer.startPlayingFile(random_file.name())) {
    Serial.print("Failed to open mp3 file:"); Serial.println(random_file.name());
    while (1);
  }
  else {
    Serial.print("Started playing: "); Serial.println(random_file.name());
  }
  random_file.close();
  int stripVal = random(switchMin,switchMax);
  while (musicPlayer.playingMusic) {
    uint16_t currentVal[14] = { 0 };
    uint16_t peak[14] = { 0 };
    musicPlayer.readFrequencies(currentVal, peak);
    int normalisedVal = (currentVal[frequencyBand]*middleNum)/11;
    for (int i = middleNum; i < numOfPixels; i++) {
      if (i-middleNum < normalisedVal) {
        if (stripVal == 1) {
          strip1.setPixelColor(i, colourArray[0], colourArray[1], colourArray[2]);
          strip1.setPixelColor((numOfPixels-1)-i, colourArray[0], colourArray[1], colourArray[2]);
        }
        else if (stripVal == 2) {
          strip2.setPixelColor(i, colourArray[0], colourArray[1], colourArray[2]);
          strip2.setPixelColor((numOfPixels-1)-i, colourArray[0], colourArray[1], colourArray[2]);
        }
        else if (stripVal == 3) {
          strip3.setPixelColor(i, colourArray[0], colourArray[1], colourArray[2]);
          strip3.setPixelColor((numOfPixels-1)-i, colourArray[0], colourArray[1], colourArray[2]);
        }
      }
      else {
        if (stripVal == 1) {
          strip1.setPixelColor(i, 0, 0, 0);
          strip1.setPixelColor((numOfPixels-1)-i, 0, 0, 0);
        }
        else if (stripVal == 2) {
          strip2.setPixelColor(i, 0, 0, 0);
          strip2.setPixelColor((numOfPixels-1)-i, 0, 0, 0);
        }
        else if (stripVal == 3) {
          strip3.setPixelColor(i, 0, 0, 0);
          strip3.setPixelColor((numOfPixels-1)-i, 0, 0, 0);
        }
      }
    }
    strip1.show();
  ...
  while (delayCounter < maxIntervalTime) {
...
int numberOfMP3s(File dir)
{
  ...
  Serial.print(F("Number of MP3s on SD Card: "));
...
void colourSelection() {
  int counter = 0;
  while (counter < colourSelectionTime) {
    int analogVal = analogRead(COLOUR_PIN);
    // analogVal = 1021;
    if (analogVal > 1020) {
      colourArray[0] = 255;
      colourArray[1] = 255;
      ...
      if (analogVal < 512) {
        colourArray[0] = 255 * sin(radians(90+90*(float)analogVal/512));
        colourArray[2] = 0;
      }
      else {
        colourArray[0] = 0;
        colourArray[2] = 255 * sin(radians(90*(float)(analogVal-512)/512));
      }
      colourArray[1] = 255 * sin(radians(180*(float)(analogVal)/1023));
    }
    updateColours(colourArray);
    delay(20);
    counter = counter+20;
  }
  updateColours(offArray);
...
void updateColours(float arr[]) {
  for(int i = 0; i < numOfPixels; i++) {
    strip1.setPixelColor(i, arr[0], arr[1], arr[2]);
    strip2.setPixelColor(i, arr[0], arr[1], arr[2]);
    strip3.setPixelColor(i, arr[0], arr[1], arr[2]);
  }
  strip1.show();
  strip2.show();
  strip3.show();
